...
if min(ct.data(:)) ==0
    'warning: ct data needs offset, applying it'
...
%% goal now is for each element
% trace the line from the element to the focus, summing the ct data along
% the way
% drawing inspiration from acoustic lens code
[Nx,Ny,Nz] = deal(dim(1),dim(2),dim(3));
[xF,yF,zF] = deal(focus_pos(1),focus_pos(2),focus_pos(3));

trace = zeros([Nx Ny Nz]);
skullthick = zeros([Nx Ny Nz]);
h=waitbar(0,'Ray tracing');

skullthresh = 1500; %m/s
radius =72; %xdcr radius mm
radpix = round(radius/ct.info.PixelDimensions(1));
%iterate through points of transGrid
for k=1:Nz
    for j=1:Ny
        for i=1:Nx
            if Egrid(i,j,k)==1
                %we are at a point on the transducer surface
                %the corresponding lens distance is dLens(j,k)
                %this distance points to the geometic focus
                %compute unit vectoir ponting from transducer
                %to geometric focus
                uvec = [i-xF, j-yF, k-zF];
                mag = sqrt(sum(uvec.^2));
                uvec = uvec./mag;
                %now slowly move along unit vector direction
                % adding values to lens until you have traveled the
                %thickness of the lens at this point
                
                %move slowly along unit vector direction
                % summing new SOS of voxels as you go
                
                % lower values are finer precision (.1 works as good as .02
                % for this)
                iter = .1; %value to scale unit vector by incrementally
                curr=1;
                d = 0;
                fooprev=[-1000 -1000 -1000];
                path = 0; % stores total speed of sound
                skullcount = 0;
                while d<=radpix
                    foo = round(curr.*iter.*uvec);
                    if sum(foo == fooprev)==3
                        %check sum to see if foo is same as last foo
                        %keeps from counting same voxel twice
                        curr = curr+1;
                    else
                        path = path + medium.sound_speed(i-foo(1), j-foo(2), k-foo(3));
                        curr=curr+1;
                        d = sqrt(sum(foo.^2));
                        fooprev = foo;
                        
                        %tick if its skull
                        if medium.sound_speed(i-foo(1), j-foo(2), k-foo(3)) > skullthresh
                ...
                trace(i,j,k)=path;
                ws = 6;
                skullthick(i-ws:i+ws,j-ws:j+ws,k) = skullcount*ct.info.PixelDimensions(1);
            end
        end
    end
    waitbar(k/Nz,h);
...
% project
close all
foo = sum(skullthick,3);
contrast = [1 4];
% subplot(121)
% imagesc(imgaussfilt(sum(trace,3),4))
% subplot(122)
imagesc(foo,contrast);
title('skullthickness 801395 (mm)')
colorbar
axis off
axis equal
%% color whole elements
